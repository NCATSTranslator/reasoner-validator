...
This documentation describes the use and API for the  5.*.* releases of the Reasoner-Validator Library.
...
Note that the functioning of this release is not backwards compatible with releases earlier than 5.0.0 the package.
...
You can optionally, `use a tool like pyenv to set your local shell Python version to a 3.12 release <https://python-poetry.org/docs/managing-environments/>`_  prior to the poetry installation.
        ...
        "warnings": {
            "warning.edge.predicate.non_canonical": {
                ...
                "biolink:participates_in": [
                    # List of distinct message parameter blocks
                    {   # the secondary context is the 'edge_id'
                        "edge_id": "a--['biolink:participates_in']->b",
                    }
                ],
                {...}  # other predicate indexed messages (same code type)
            },
...
Every message has a 'code' and optional sets of context-specific parameters which correspond to
...
.. code-block:: json

    {
        ...
        # Otherwise, a TRAPI Response embedded 'schema_version' (not 'latest') becomes the default validation version."

        "trapi_version": "1.6.0",

        "_comment": "If the Biolink Model version is omitted or set to None, then the current Biolink Model Toolkit is used.

        # Note: for TRAPI releases from 1.6.0 onwards, the Response message will state the assumed 'biolink_version'.
        # This modifies slightly the interpretation of this parameter, as follows:
        # If the 'biolink_version' given here is assumed, which overrides the TRAPI Response stated 'biolink_version';
        # Otherwise, a TRAPI Response embedded 'biolink_version' (not BMT) becomes the default validation version.
...
* 5.0.# updated to TRAPI 1.6.0; removed all support for TRAPI earlier than 1.5.0; removed source audit trail annotation (but still validates the basic source provenance itself)
